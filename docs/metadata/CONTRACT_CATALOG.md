...
| Agent runtime (`metadata/agent_runtime.py`) | `RuntimeBudgetMetadata`, `ProviderBudgetDiagnostic`, `ContextSelectionMetadata`, `EditPlanMetadata`, `VerificationPlanMetadata`, `PathIntentMetadata`, `PathResolutionMetadata`, `DecisionNeedMetadata`, `ToolDecisionMetadata`, `GuardDecisionMetadata`, `RuntimeStateMetadata`, `RuntimeCheckpointMetadata`, `RuntimeResumeDecisionMetadata`, `RuntimeReportMetadata` | Mutable phase-driven task state, strict per-attempt budget evidence, prompt-context selection, bounded tool/recovery and tool-event completion budgets, evidence-backed decisions, path grounding, durable recovery checkpoints and finalization cursor, explainable resume preflight, and the final derived report. `EnhancementCompletionPurposeLimit` owns separate initial and recovery ceilings; `EnhancementCompletionRequest.max_tokens_cap` binds a reservation to an explicitly configured provider output limit, while reservation/reconciliation IDs remain the sole recovery ledger. `ActiveDiagnosticConflict`, `ActiveDiagnosticRisk`, and `ActiveDiagnosticDecision` are strict task-owned nested values: they add conflict/risk lifecycle and the Controller's measure/act/verify/recover/stop choice without replacing `DecisionNeedMetadata`, `ToolDecisionMetadata`, Guard, or project diagnosis. `CoreAcceptanceDecision` is a bounded, source-linked nested value; only typed passed or user/goal-policy waived decisions can close an active acceptance constraint. `CoreCompletionHandoffView`, `CoreCompletionSourceReferences`, `CoreCompletionPackageView`, its budget summary, and build result are frozen derived values without a `MetadataKind`: the unique stateless builder revalidates checkpoint/report/state/project sources and never persists a second core truth. The latest diagnostic progress signature is a derived identity over authoritative content, not a second fact store. `RuntimeTaskPurpose` distinguishes normal project completion from read-only response-evidence collection. `DecompositionPolicyDecision` is a strict cursor/state-owned value for single/initial/local/replan admission; neither is a new public metadata kind. `ContextSelectionMetadata.compaction_attempts` nests body-free `ContextCompactionAttempt` values owned by the same assembly snapshot; they distinguish provider acceptance from builder selection without creating a second compact authority. |
...
- `CoreCompletionHandoffView` and `CoreCompletionPackageView` are recomputed,
  frozen projections over those owners. Their hashes and bounded references
  support integration/replay, but neither may update source facts or substitute
  for the final checkpoint/report. The package has one builder in
  `autonomous_iteration/core_completion_package.py`.
- Active diagnostic conflicts/risks are owned by `RuntimeStateMetadata` and
  copied only into the derived runtime report. `ProjectDiagnosisMetadata`
  remains project-domain diagnosis, `GuardDecisionMetadata` remains admission
  evidence, and experiment-local mini-SWE `ActiveState` remains outside the
  production metadata tree. `ActiveDiagnosticDecision` owns the Controller's
  next-step category; the concrete need, tool capability, Guard outcome, and
  execution receipt retain their existing distinct owners.
